...
2. **`app/schemas/` is generated by CI, not committed by hand.** Download the
   `room-schema` artifact from a green CI run and commit the JSON. Both `1.json`
   and `2.json` are committed as of the 1.0.0 integration; `1.json` stays because
   it is the starting point `MIGRATION_1_2` has to migrate from.
...
4. **`update.json` / APK filename mismatch — fixed.** `release.yml` used to
   advertise `.../scope-wa.apk` while uploading AGP's `app-release.apk`, so the
   in-app updater would have 404'd on the very first check. The workflow now
   renames the APK to `scope-wa.apk` before upload, rather than correcting the
   URL, because that URL is baked into every client the moment 1.0.0 ships and a
   shipped updater cannot be told to look elsewhere.
5. **The release APK's signature is now verified, not assumed.** `release.yml`
   runs `apksigner verify` on the APK whenever the signing secrets were present,
   and fails the job if it is unsigned — a misconfigured keystore used to be able
   to publish an uninstallable APK under a `v1.0.0` tag that cannot be reused.
6. **Minification is off for 1.0.0**, deliberately — see the comment on
...
- **The bottom bar holds five items; everything else lives behind More.** Home,
  Contacts, Templates and Campaign — the path through running a campaign — keep
  tabs. Extract, Group Add, Activity log, Setup & permissions and Diagnostics are
  one tap away in `ui/more/MoreScreen.kt`, and the More tab stays lit while you
  are on one of them. Phases 4, 6 and 7 each appended to the bar independently
  and it had reached seven items, past Material's 3–5 for `NavigationBar`;
  worse, Phase 7's Group Add had **no** bar entry and nothing else linked to it,
  so a finished screen shipped unreachable. **If you add a top-level screen, add
  it to `MoreScreen` — do not grow the bar.**
